...
> **Last updated:** 2026-07-06 (v3.0.2 on branch `v3.0.2`: auto-update lands in place via `/DIR` — fixes the "profiles deleted" illusion on git-based installs — and the update flow is a clean window handoff (old window closes itself, relaunched AMM opens the new one). Same day: v3.0.1 fresh-batch detection; v3.0.0 dashboard redesign. See "Recent change history" below — earlier sections of this file may lag a version or two behind.)
