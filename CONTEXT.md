...
> **Last updated:** 2026-07-06 (v3.0.1 on branch `v3.0.1`: fresh-batch detection — the dashboard auto-refreshes and raises a "Scrape complete" banner when ANY scrape finishes (scheduler/Telegram/tray/dashboard), via `generatedAt` on the 5s status poll. Front-end only. v3.0.0 same day: ground-up dashboard redesign into an app-shell UI. See "Recent change history" below — earlier sections of this file may lag a version or two behind.)
