...
> **Last updated:** 2026-06-11 (v2.0 audit remediation on branch `v2.0`, rebased onto v1.3.0 main: /update alias collision fixed after 5 broken releases, Security+/C++ scoring fix via term-match.mjs, setup-tasks.ps1 PS5.1 encoding parse-failure fixed, wizard bot-start verification, scrape lock, update rollback, 409 detection, log rotation, process.execPath self-spawns + gitCmd(), wizard/file-picker timeouts, installer de-silencing, 11 new tests, npm run check + CI action bumps. Version → 2.0.0.)
