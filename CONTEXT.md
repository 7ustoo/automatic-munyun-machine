...
> **Last updated:** 2026-07-09 (v4.2.0 on branch `v4.1.1`: first-run onboarding — welcome splash, skippable guided setup with a "Finish setup" nudge banner, titles↔keywords preview toggle, and a post-finish coach-mark tour; PLUS the `.exe` installer now bundles a portable Node runtime at `{app}\runtime\node.exe` — fixes "Could not run the helper. Is Node installed?" on machines without system Node. Prior: v4.1.0 on branch `v4.1`: hiring.cafe sign-in status card on the dashboard System page + funnel transparency — see "Recent change history". Prior: v4.0.0 on branch `v4.0`: matching overhaul — two-pass scoring on the FULL job description, ambiguous-term guard ("Palo Alto, CA" fix), role-family gate, salary as tie-breaker, opt-in Claude AI rerank (`scripts/ai-rerank.mjs`), Why-panel score journey + misses with +search/mute; reliability — scrape-status.json + red failure banner + scrapeRunning, config snapshots/restore (data/backups/), sha256-verified updates, applied-marks in localStorage, dashboard static-contract CI test + committed behavioral harness (dev/dashboard-harness, npm run test:ui). Same day: v3.0.x. See "Recent change history".)
...
| `dashboard.html` | **v3.0 rewrite** (v1.3 origin). Single self-contained dark-theme app (inline CSS/JS/SVG icons, no CDN — fully offline). App shell with sidebar + hash-routed views (Jobs/Searches/Resume/Profiles/System/Settings), jobs explorer (search `/`, band filter, sort, match meters, YOE + source-query columns), custom modal system (no native confirm/prompt), skeleton/empty states, scrape progress bar, restyled 5-step setup overlay. All v2.x element IDs, API payloads, and gating semantics preserved; `__AMM_TOKEN__` placeholder exactly once (asserted by `dashboard_actions_test.go`). Status poll every 5s unchanged. |
...
| `LICENSE` | MIT. |
| `package.json` | npm manifest. v4.2.0. Deps: `playwright-core`, `pdf-parse`, `mammoth`. |
...
- **2026-07-09** — v4.2.0 (branch `v4.1.1`). **First-run onboarding overhaul (dashboard.html).** The v2.7 5-step setup overlay gained a front door and an escape hatch. New **step 0 welcome splash** (`.setup-welcome`, data-step="0") — value prop + "Let's set it up" / "Skip for now"; progress dots + Back/Next nav hidden on it (`setupShowStep` handles `n===0`, `setupCanProceed(0)→true`). **Skip** (`setupSkip()`): POSTs `{}` to `/api/setup/init` (buildInitConfig backfills from config.example.json), sets `localStorage.ammSetupIncomplete`, reloads — deliberately skips `setup-finalize` so no scheduler/failing 7am scrape. On the normal dashboard, `setupNudges()` shows a persistent yellow **#setup-banner** ("Finish setup" → `reopenSetup()` re-shows the overlay; `setupWire` is now re-entrant via `__setupWired`). **Step 1 reframed** as "here's what we'll hunt for" with a **titles↔keywords `#setup-mode-toggle`** — the scan logic moved into `setupScanResume(modeOverride)`, which appends an optional `mode` to `/api/resume/upload`; new `mode` plumbing: Go `handleResumeUpload` reads `r.FormValue("mode")` → passes as 5th relay arg → `resumeParse(a, b)` honors `titles|keywords` over the fresh-install default. **Post-finish coach-mark tour** (`#tour-layer`/`#tour-hl`/`#tour-pop`, `TOUR_STEPS` × 4: `#nav`, `#scrape-btn`, `.stats`, `[data-view=system]`) armed by `setupFinish` via `localStorage.ammTourPending`, run on next boot. All green: 167 node tests incl. dashboard-static contract (new ids reconcile, no dupes, offline, token×1); go build/vet/test; `go` + syntax clean. UI harness behavioral checks pass (its `HARNESS: FAIL` is a pre-existing stub favicon 404, reproduced on a clean tree).
- **2026-07-08** — v4.1.1 work, shipped as part of v4.2.0 (branch `v4.1.1`). **Bundled Node runtime — fixes "Is Node installed?" on fresh `.exe` installs.** Root cause: `AMM.exe` execs Node helpers for every dashboard action (`findNode()` in `wrapper/supervisor.go`, used by `dashboard_actions.go` + the bot spawn), but the Inno installer only shipped `node_modules`, never the Node *runtime* — so on a machine with no system Node, Scrape/Sign-in/update all failed with "Could not run the helper. Is Node installed?" (`dashboard_actions.go:106`). The `iwr|iex` path was fine — `install.ps1:29-44` winget-installs Node. Fix: the release build (`.github/workflows/release.yml`, `build-windows`) downloads Node v20 win-x64, verifies it against Node's official `SHASUMS256.txt`, and stages `node.exe` at repo-root `runtime/` (gitignored) so Inno's `Source: "..\*"` bundles it to `{app}\runtime\node.exe`. New resolution order everywhere Node is spawned, bundled runtime first: `findNode()` gains `bundledNode()` (resolves `<AMM.exe>\..\..\runtime\node.exe` off `os.Executable()`); `start-bot.cmd` / `run-daily-batch.cmd` prepend `%ROOT%\runtime\node.exe`; `amm.iss` Chromium post-install now runs `runtime\node.exe node_modules\playwright-core\cli.js install chromium` (fresh machines have no `npx`, and only `playwright-core` ships) and the uninstall step calls the bundled node too. System Node stays a fallback. Adds ~30 MB (compressed) to the installer.
- **2026-07-07** — v4.1.0 (branch `v4.1`). **hiring.cafe sign-in status + funnel transparency.** (1) Dashboard System page gains a hiring.cafe sign-in status card: `dashboard-api.mjs` subcommands `hcafe-auth-get` (reads cached `data/hcafe-auth.json`) / `hcafe-auth-check` (`job-action.mjs auth` re-probe), wrapper routes `GET /api/hcafe/auth` + `POST /api/hcafe/auth/refresh`, with a Sign-in button. (2) Funnel transparency: every batch now reports `raw → after filters → fresh → above floor → delivered` — a `funnel` object in `data/last-batch.json` surfaced via `/api/batch`, rendered as `funnelLine`/`funnelLineJS` on the dashboard and in the Telegram message + `jobs(date).txt` header.
- **2026-07-06** — v4.0.0 (branch `v4.0`). **Matching + reliability overhaul.** Scoring (daily-batch.mjs): pass-1 card shortlist (130) → `resolveOnePage` now also captures `document.body.innerText` (7k cap) → pass-2 rescore on real JD (`jdScoreToPercent` bands, final = .4·card + .6·jd, `scoring.jdRescore` default on) → optional pass-3 `aiRerank()` (top 40, one structured-output Claude call, blend .45 kw/.55 ai, fail-open, key at `scoring.ai.apiKey` — never returned/logged, settings expose `aiHasKey` only). False-positive kills: `AMBIGUOUS_TERM_CONTEXT`/`termAllowedInText` (palo alto/chef/puppet/salt need context), `OFF_FAMILY_RX` ×0.35 title gate (escapes: cluster term in title, or no CV clusters), salary → `compareJobs`/`salaryRank` tie-break (bonus/penalty dead). `missingTerms()` scans full cv-keywords dict for JD asks the CV lacks; `scoring.mutedTerms` skips terms (new `score-mute` subcommand + POST /api/score/mute). last-batch jobs += cardPct/jdPct/aiPct/aiReason/missing/salaryK. Reliability: `writeScrapeStatus()` → data/scrape-status.json on every exit path; Go `readScrapeStatusInto` adds `lastScrape` + `scrapeRunning` (scrape.lock.lock mtime <90s) to /api/status; dashboard red FAILED banner + Try again + bar for externally-started scrapes. `snapshotConfig/list/restore` in config-rw (data/backups, keep 10; snapshot before setup-init/profile rename/delete) + Profiles-page restore UI (GET /api/config/backups, POST /api/config/restore). self-update verifies installer sha256 against SHA256SUMS.txt (abort on mismatch/unverifiable). Applied-marks persist per-batch in localStorage; thin-CV warning (<8 terms) in Resume card; Smart match settings card. Tests: 162 node (v4-scoring.test 9, dashboard-static.test 5 — the id/CSRF/symbol/offline contract now in CI) + 4 Go; behavioral harness committed to dev/dashboard-harness (`npm run test:ui`) — all suites re-passed incl. fail-banner + Why-journey assertions. Deferred: TG-setup UI dedupe, selector centralization.
- **2026-07-06** — v3.0.2 (branch `v3.0.2`). **Update integrity + window handoff.** Two user reports. (1) **"Profiles deleted on update"** — root cause: `self-update.mjs` ran the installer `/VERYSILENT` with no `/DIR`; on git-based installs (no Inno registry dir entry — e.g. `install.ps1` clones, including the dev machine where the repo IS the live install) Inno fell back to `DefaultDirName` = `%LOCALAPPDATA%\automatic-munyun-machine`, installing a FRESH copy and relaunching it: no config.json → setup screen → "profiles gone." Nothing was deleted — data stayed in the original dir while the wrong copy ran. Fix: `/DIR="<ROOT>"` on the installer line so updates land in place. Caveat: takes effect for updates applied FROM ≥3.0.2 (the update TO 3.0.2 runs the old updater — git installs should `git pull` this one). (2) **Stale window after update** — the app window is a separate Chrome process (own `--user-data-dir`) the installer's taskkill never reached; the old window lingered next to the relaunched one. Two independent close mechanisms: (a) wrapper-side — `openAppWindow` records the spawned PID (`recordAppWindowPID`, then Release as before); `handleUpdateApply` parses the relayed JSON and on `ok+started` fires `closeAppWindows()` after 4s (2 new Go tests: real child killed, dead-PID no-op); (b) page-side — internal nav switched from hash-pushes to `history.replaceState` (`navTo()`, nav-click interception; keeps `history.length===1`, Chrome's precondition for `window.close()` on app windows), update-apply sets `updatingState="applying"`, and 2 consecutive failed status polls → `#update-splash` + `window.close()` (honest fallback copy if refused). Error banner suppressed while updating — wrapper death is the install working. Verified: production-faithful Playwright test (launchPersistentContext `--app`, history stays 1 across nav) — click Update, kill stub → splash + window closes ITSELF; fresh-batch banner regression re-passed; 153 node + Go green; static check 121 ids.
- **2026-07-06** — v3.0.1 (branch `v3.0.1`). **Fresh-batch detection — the dashboard notices every scrape.** User report: scheduled/Telegram/tray scrapes finished silently; only the dashboard's own Scrape button triggered a refresh (its private 12×12s poll loop), so 7am batches required a manual refresh. Fix rides the existing 5s status poll: `render()` tracks `s.lastBatch.generatedAt` in `state.lastGen` (null = no baseline; first tick records only, so a page load never celebrates an old batch); on change → `onFreshBatch(lb)` = `loadBatch()` (list + stat tiles auto-refresh) + green dismissible `#batch-banner` ("Scrape complete — N jobs ranked for date" + **View jobs** → `#jobs` + dismiss ×) + toast + clears `#scrapebar`. Scrape button's private polling loop deleted — the detector owns completion for ALL sources; button now just starts the bar with a 45-min safety clear (`scrapeSafetyTimer`, matches the scheduled run's budget). `generatedAt` was already in `/api/status` (`dashboard.go:462`) — zero backend changes. Verified behaviorally: stub server flips `generatedAt` at +15s → Playwright run with zero clicks shows 27→42 auto-refresh, banner + toast raised, dismiss works, no console errors. Static check 118 ids OK; 153 node + Go suites green. Also fixed in passing: v3.0.0's CONTEXT entry had swallowed the v2.9.0 entry header (concatenation from a bad edit) — restored.
- **2026-07-06** — v3.0.0 (branch `v3.0`). **Dashboard redesign, front-end only.** `wrapper/dashboard.html` rewritten (1,569 → ~2,600 lines) from a single-scroll card dump into an app shell: fixed sidebar (6 hash-routed views: `#jobs #searches #resume #profiles #system #settings`, refresh-safe), topbar (profile chip · health chip · 👁 Watch · Scrape now), and a jobs explorer — stat tiles (count/avg/strong≥70/date from `/api/batch` meta), client-side search (`/` shortcut) + match-band segmented filter + 5-way sort, per-row match meter + YOE + source-query pill (fields `yoe`/`q` were always in `batchJob` but never rendered), session-sticky applied/why state in a JS `state` object so re-renders and mid-scrape polls don't reset them. Native `confirm()`/`prompt()` (3 sites) replaced by a promise-based modal component (Esc/Enter/backdrop, focus-return). Scrape polling gains an indeterminate `#scrapebar` + early-exit on `generatedAt` change. Export = `<details>` dropdown (same 3 GET links). Setup overlay restyled (numbered labeled stepper) with wiring verbatim from v2.7. Icons = inline SVG `<symbol>` set; zero external resources. **No Go/Node/API/schema changes** — contract pinned by: static check (114 referenced IDs all present, token placeholder ×1, symbols defined), `go test ./...` green (incl. token-injection test), 153 node tests green, and a 12-screenshot Playwright pass against a stub wrapper (all views + modal + setup + narrow viewport, zero console errors). Stub server + checker live in the session scratchpad, not the repo.
