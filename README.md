...
- **Email the batch to a helper (v4.3).** Hand off applying to someone else — a virtual assistant, say. Connect your Gmail once (System → **Email**), then click **Email** next to Export to send the day's `jobs(YYYY-MM-DD).txt` to your chosen recipient, or turn on **Auto-send** to email it automatically every morning after the scrape. Gmail needs a free **App Password** (2-Step Verification on, then create one at [myaccount.google.com/apppasswords](https://myaccount.google.com/apppasswords)) — your normal password won't work over email. Your password is stored only on your computer (in `.env`), never uploaded.
